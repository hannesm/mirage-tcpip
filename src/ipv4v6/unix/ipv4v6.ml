...
let mtu _ ~dst = match dst with
  | Ipaddr.V4 _ -> 1500 - 20 (* Ipv4_wire.sizeof_ipv4 *)
  | Ipaddr.V6 _ -> 1500 - 40 (* Ipv6_wire.sizeof_ipv6 *)

let disconnect _ = Lwt.return_unit
let connect ~ipv4_only:_ ~ipv6_only:_ ipv4 ipv6 =
  Lwt.return { ipv4 ; ipv6 }
...
let get_ip { ipv4 ; ipv6 } =
  List.map (fun ip -> Ipaddr.V4 ip) (Ipv4.get_ip ipv4) @
  List.map (fun ip -> Ipaddr.V6 ip) (Ipv6.get_ip ipv6)
