...
type ipaddr   = Ipaddr.t
  ...
  type error = [ Tcpip.Ip.error | `Ipv4 of Static_ipv4.error | `Ipv6 of Ipv6.error | `Msg of string ]

  let pp_error ppf = function
    | #Tcpip.Ip.error as e -> Tcpip.Ip.pp_error ppf e
    | `Ipv4 e -> Static_ipv4.pp_error ppf e
    | `Ipv6 e -> Ipv6.pp_error ppf e
    | `Msg m -> Fmt.string ppf m

  type t = { ipv4 : Static_ipv4.t ; ipv4_only : bool ; ipv6 : Ipv6.t ; ipv6_only : bool }
    ...
    fun buf ->
      if Cstruct.length buf >= 1 then
        ...
        if v = 4 && not t.ipv6_only then
          Static_ipv4.input t.ipv4 ~tcp:tcp4 ~udp:udp4 ~default:default4 buf
  ...
  let write t ?fragment ?ttl ?src dst proto ?size headerf bufs =
    ...
    | Ipaddr.V4 dst ->
      if not t.ipv6_only then
        ...
        | Ok src ->
          Static_ipv4.write t.ipv4 ?fragment ?ttl ?src dst proto ?size headerf bufs >|= function
          | Ok () -> Ok ()
  ...
  let pseudoheader t ?src dst proto len =
    ...
    | Ipaddr.V4 dst ->
      ...
      in
      Static_ipv4.pseudoheader t.ipv4 ?src dst proto len
    | Ipaddr.V6 dst ->
      let src =
        match src with
        | None -> None
  ...
  let src t ~dst =
    match dst with
    | Ipaddr.V4 dst -> Ipaddr.V4 (Static_ipv4.src t.ipv4 ~dst)
    | Ipaddr.V6 dst -> Ipaddr.V6 (Ipv6.src t.ipv6 ~dst)

  let get_ip t =
    List.map (fun ip -> Ipaddr.V4 ip) (Static_ipv4.get_ip t.ipv4) @
    List.map (fun ip -> Ipaddr.V6 ip) (Ipv6.get_ip t.ipv6)

  let mtu t ~dst = match dst with
    | Ipaddr.V4 dst -> Static_ipv4.mtu t.ipv4 ~dst
