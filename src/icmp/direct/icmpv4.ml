...
module Log = (val Logs.src_log src : Logs.LOG)
  ...
  type error = [ `Ip of Ipv4.error ]
  let pp_error ppf (`Ip e) = Ipv4.pp_error ppf e
  ...
  let writev t ?src ~dst ?ttl bufs =
    Ipv4.write t.ip ?src dst ?ttl `ICMP (fun _ -> 0) bufs >|= function
    | Ok () -> Ok ()
    | Error e ->
      Log.warn (fun f -> f "Error sending IP packet: %a" Ipv4.pp_error e);
      Error (`Ip e)
    ...
    | Ok (message, payload) ->
      ...
      | Echo_request, Id_and_seq (id, seq) ->
        ...
        if t.echo_reply then begin
          ...
          | Ok () -> ()
          | Error (`Ip e) ->
            Log.warn (fun f -> f "Unable to send ICMP echo-reply: %a" Ipv4.pp_error e); ()
