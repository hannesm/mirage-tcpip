...
let connect ipv4v6 =
  ...
  let interface =
    ...
    | [ Ipaddr.V4 ip ], [] ->
      ...
      `V4_only v4_unix
    | [], [ Ipaddr.V6 ip ] ->
      `V6_only (Ipaddr_unix.V6.to_inet_addr ip)
    | [ Ipaddr.V4 ipv4 ], [ Ipaddr.V6 ipv6 ] ->
      (match Ipaddr.V6.(compare ipv6 unspecified = 0), Ipaddr.V4.(compare ipv4 any = 0) with
       | true, true -> `Any
       | _, _ ->
  ...
  | Ok (family, proj) ->
    ...
    Lwt.catch (fun () ->
        ...
        ( match keepalive with
          | None -> ()
          | Some { Tcp_keepalive.after; interval; probes } ->
