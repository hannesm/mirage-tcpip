...
module Ip = Ipv4v6
  ...
  type error = [ Tcp_error.read | WIRE.error]

  let pp_error ppf = function
    | #Tcp_error.read as e -> Tcp_error.pp_read ppf e
    | #WIRE.error as e -> WIRE.pp_error ppf e

  type write_error = [ Tcp_error.write | `Not_ready ]

  let pp_write_error ppf = function
    | `Not_ready ->
      Fmt.string ppf "attempted to send data before connection was ready"
    | #Tcp_error.write as e -> Tcp_error.pp_write ppf e
  ...
  type t = {
    ip : Ip.t;
    listeners : (int, Tcp_keepalive.t option * (flow -> unit Lwt.t)) Hashtbl.t ;
    mutable active : bool ;
    ...
    connects: (WIRE.t, ((connection, error) result Lwt.u * Sequence.t * Tcp_keepalive.t option)) Hashtbl.t;
  }
